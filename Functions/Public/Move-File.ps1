        ...
        try
        {
            if ($OldLocation -AND $NewLocation ) { 
                if ((Test-Path -Path $NewLocation) -eq $true) {
                    ...
                    Write-Output -Message ('The Destination Directory:{0} does not exist' -f $NewLocation)
                    Return
                }
            }
        }
